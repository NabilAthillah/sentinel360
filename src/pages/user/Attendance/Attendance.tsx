...
const Attendance = () => {
    ...
    const [nowSG, setNowSG] = useState<{ hhmmss: string; minutes: number }>({ hhmmss: '00:00:00', minutes: 0 });

    const [cta, setCta] = useState<{ label: string; disabled: boolean }>({ label: "Let's check in", disabled: true });
    ...
    const getSetting = (label: string) =>
        attendanceSetting.find((s) => s.label.trim().toLowerCase() === label.trim().toLowerCase())?.value ?? null;
    ...
    const decideReliefShift = (): 'relief-day' | 'relief-night' | null => {
        ...
        if (rdS != null && rdE != null && inRange(nowMin, rdS, rdE)) return 'relief-day';
        if (rnS != null && rnE != null && inRange(nowMin, rnS, rnE)) return 'relief-night';
        return null;
    };
    ...
    const pickNearestSiteWithin = (sites: MiniSite[], me: Coords, maxDistMeters: number) => {
        ...
        for (const s of sites) {
            ...
            if (dist <= maxDistMeters && (!best || dist < best.dist)) best = { site: s, dist };
        }
        return best;
    };
    ...
    const parseEmployeeShift = (raw: any): Shift | null => {
        ...
        if (s === 'relief day' || s === 'relief-day' || s === 'relief_day') return 'relief-day';
        if (s === 'relief night' || s === 'relief-night' || s === 'relief_night') return 'relief-night';
        return null;
    };
    ...
    const diffHM = (startISO: string, endISO: string) => {
        ...
        if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) return '0h0m';
    ...
    const nowSingaporeParts = () => {
        ...
        return { hhmmss: `${two(hh)}:${two(mm)}:${two(ss)}`, minutes: hh * 60 + mm };
    };
    ...
    const getShiftStartEndMin = (shift: typeof currentShift) => {
        ...
        if (shift === 'day') {
            return { start: timeToMin(getSetting('Day shift start time')), end: timeToMin(getSetting('Day shift end time')) };
        }
        if (shift === 'night') {
            return { start: timeToMin(getSetting('Night shift start time')), end: timeToMin(getSetting('Night shift end time')) };
        }
        if (shift === 'relief-day') {
            return { start: timeToMin(getSetting('RELIEF Day shift start time')), end: timeToMin(getSetting('RELIEF Day shift end time')) };
        }
        if (shift === 'relief-night') {
            return { start: timeToMin(getSetting('RELIEF night shift start time')), end: timeToMin(getSetting('RELIEF night shift end time')) };
        }
    ...
    const headerTimeText = useMemo(() => {
        const map = {
            'day': [getSetting('Day shift start time'), getSetting('Day shift end time')],
            'night': [getSetting('Night shift start time'), getSetting('Night shift end time')],
            'relief-day': [getSetting('RELIEF Day shift start time'), getSetting('RELIEF Day shift end time')],
            'relief-night': [getSetting('RELIEF night shift start time'), getSetting('RELIEF night shift end time')],
        } as const;
    ...
    const shiftIconKind = useMemo<'sun' | 'moon' | null>(() => {
        ...
        return (currentShift === 'day' || currentShift === 'relief-day') ? 'sun' : 'moon';
    }, [currentShift]);
    ...
    const handleCtaClick = async () => {
        ...
        const dateISO = employeeDateISO ?? new Date().toISOString().slice(0, 10);
        const swalBase = {
            background: '#1e1e1e',
            color: '#f4f4f4',
        ...
        try {
            ...
            if (!hasIn) {
                ...
                await Swal.fire({
                    ...
                    text: res?.message ?? (payload.early ? 'Reason submitted. You have checked out early.' : 'Successfully checked out.'),
                });
            }

            const refreshed = await attendanceService.getAttendanceBySiteUserShift(token, {
                site_id: currentSiteId,
                user_id: user.id,
                shift: toApiShift(currentShift),
                date: dateISO,
    ...
    const runNearestFlow = async (token: string) => {
        if (geoRadius <= 0) {
            ...
            setDecision({ canAttend: false, siteSource: 'none', reason: 'No siteId & nearest disabled' });
            return;
        }
        ...
        if (!nearest) {
            ...
            setDecision({ canAttend: false, siteSource: 'none', reason: 'No siteId & no nearest site' });
            return;
        }
            ...
            if (user) {
                ...
                if (seRes?.success && seRes?.data) {
                    ...
                    if (empShift && seSiteId === String(nearest.site.id)) {
                        ...
                        const seDate = (seRes?.data?.date ?? seRes?.data?.site_employee?.date ?? seRes?.data?.siteEmployee?.date) as string | undefined;
        ...
        if (rel) {
            ...
            setDecision({ canAttend: false, siteSource: 'nearest', reason: 'Current time does not fall into Relief Day/Night' });
        }
    };

    const initAttendanceDecision = async () => {
        const token = localStorage.getItem('token');
        if (!token) { console.warn('[attendance] no token'); return; }
        if (!settingsReady) { console.log('[attendance] settings not ready yet'); return; }
            ...
            if (siteIdParam) {
                ...
                if (user) {
                    try {
                        ...
                        console.error('[attendance] getNearestSiteUser error (param branch):', e);
                    }
                }
    ...
    useEffect(() => {
        ...
        run();
    }, [decision?.canAttend, currentSiteId, currentShift, user?.id, employeeDateISO]);
    ...
    useEffect(() => {
        if (!currentShift) { setCta({ label: '-', disabled: true }); return; }
        const { start, end } = getShiftStartEndMin(currentShift);
        const grace = getGraceMin();
        if (start == null || end == null) { setCta({ label: '-', disabled: true }); return; }
    ...
    }, [nowSG.minutes, currentShift, attendanceSetting, checkInText, checkOutText]);
        ...
        <div className="bg-[#181D26] min-h-screen text-white flex flex-col gap-12">
            ...
            <div className="rounded-lg flex flex-col gap-6 justify-between px-6">
                            ...
                            <div className="flex items-center gap-1 text-sm text-[#F4F7FF] h-full">
                                {shiftIconKind === 'sun' ? (
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" width="16" height="16" viewBox="0 0 16 16">
                                        <path d="M4.51 3.23L3.31 2.03l-.94.94 1.19 1.19 1.94-1.93zM2.67 7H.67v1.33h2V7zM8.67.37H7.33v2h1.33v-2zM13.63 2.97l-.94-.94-1.19 1.19 1.19 1.19 1.94-1.93zM11.49 12.11l1.19 1.2.94-.94-1.2-1.19-.93.93zM13.33 7v1.33h2V7h-2zM8 3.67A4 4 0 1 0 12 7.67 4 4 0 0 0 8 3.67zM7.33 14.97h1.33v-1.97H7.33v1.97zM2.37 12.36l.94.94 1.19-1.2-.94-.94-1.19 1.2z" fill="#F3C511" />
                                    </svg>
                                ) : (
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" width="16" height="16" viewBox="0 0 16 16">
                                        <path d="M9.796 3.018c.157-.414-.21-.846-.653-.781-3.22.467-5.525 3.541-4.865 6.93.455 2.345 2.385 4.206 4.748 4.59 2.181.356 4.176-.507 5.436-2.006.28-.333.111-.864-.321-.945-3.512-.671-5.647-4.399-4.345-7.788z" fill="#33569F" />
                                    </svg>
                                )}
                                <span className="text-[#F4F7FF] capitalize">{shiftText} Shift</span>
                            </div>
                        ) : (
                            <p>-</p>
                        )}
                ...
                <button
                    ...
                    className={`rounded-full flex flex-wrap justify-center gap-3 items-center w-full py-[13.5px] bg-[#EFBF04] ${cta.disabled ? 'opacity-60 cursor-not-allowed' : 'hover:opacity-90'}`}
                >
                    {cta.label != '-' && !loading ? (
                        <>
                            ...
                            <span className="flex text-[#181D26] text-base font-bold gap-2 text-center w-fit font-inter">|</span>
                            <span className="flex text-[#181D26] text-base font-bold gap-2 text-center w-fit font-inter">
                                {nowSG.hhmmss}
                            </span>
                        </>
